...
class TruvideoSdkCameraRepositoryImpl (
    ...
    override fun takeImage(
        captureConfig: CameraCaptureConfig
    ): Flow<ImageCaptureEvent> = cameraAdapter.takeImage(captureConfig)

    override fun takeVideoSnapshot(captureConfig: CameraCaptureConfig): Flow<ImageCaptureEvent> =
        cameraAdapter.takeVideoSnapshot(captureConfig)

    override fun requestFocus(captureConfig: CameraCaptureConfig): Flow<FocusState> =
        cameraAdapter.requestFocus(captureConfig)
    ...
    override fun changeCamera(
        ...
        cameraAdapter.startCameraById(cameraId, resolution, captureConfig)
    }
    ...
    override fun restartPreview(
        ...
        cameraId: String?,
        resolution: TruvideoSdkCameraResolution?,
        captureConfig: CameraCaptureConfig) = cameraAdapter.restartPreview(
            restartCamera,
            cameraId,
            resolution,
            captureConfig)
