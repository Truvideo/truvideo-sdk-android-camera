...
internal class CameraUIReducer(
    ...
    fun handleShowMediaDetail(state: CameraUiState, media: TruvideoSdkCameraMedia) : ReducedResult<CameraUiState, CameraUiEffect> {
        ...
        logAdapter.addLog(
            ...
            message = "Open panel media detail. Media: ${media.toJson()}",
            severity = TruvideoSdkLogSeverity.INFO,
        )
    ...
    fun handleContinueButtonPressed(state: CameraUiState) : ReducedResult<CameraUiState, CameraUiEffect> {
        val mediaList = state.mediaState.media.toList()

        logAdapter.addLog(
            eventName = "event_camera_button_continue_pressed",
            message = "Button continue pressed. Result: ${mediaList.joinToString(", ")}",
            severity = TruvideoSdkLogSeverity.INFO,
        )
                ...
                effects.tryEmit(
                    CameraUiEffect.SendEvent(
                        TruvideoSdkCameraEvent(
                            ...
                            data = TruvideoSdkCameraEventMediaContinue(mediaList),
