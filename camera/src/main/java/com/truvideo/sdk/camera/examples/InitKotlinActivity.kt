...
class InitKotlinActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TruvideoSdkCamera.events.observe(this) { event: TruvideoSdkCameraEvent ->
            when (event.type) {
                TruvideoSdkCameraEventType.RECORDING_STARTED -> {
                    val data = event.data as TruvideoSdkCameraEventRecordingStarted
                }
                TruvideoSdkCameraEventType.RECORDING_FINISHED -> {
                    val data = event.data as TruvideoSdkCameraEventRecordingFinished
                    data.media.filePath
                }
                TruvideoSdkCameraEventType.RECORDING_PAUSED -> TODO()
                TruvideoSdkCameraEventType.RECORDING_RESUMED -> TODO()
                TruvideoSdkCameraEventType.IMAGE_TAKEN -> TODO()
                TruvideoSdkCameraEventType.CAMERA_DEVICE_CHANGED -> TODO()
                TruvideoSdkCameraEventType.RESOLUTION_CHANGED -> TODO()
                TruvideoSdkCameraEventType.FLASH_MODE_CHANGED -> TODO()
                TruvideoSdkCameraEventType.ZOOM_CHANGED -> TODO()
                TruvideoSdkCameraEventType.MEDIA_DELETED -> TODO()
                TruvideoSdkCameraEventType.MEDIA_DISCARDED -> TODO()
                TruvideoSdkCameraEventType.CONTINUE -> TODO()
            }
        }
    }
...
//            durationLimit = 1000 * 60 * 60
//        )
//        val mode = TruvideoSdkCameraMode.image()
//        val mode = TruvideoSdkCameraMode.image(
//            maxCount = 5
//        )
//        val mode = TruvideoSdkCameraMode.singleVideo()
//        val mode = TruvideoSdkCameraMode.singleImage()
