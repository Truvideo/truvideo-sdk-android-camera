...
class InitKotlinActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TruvideoSdkCamera.events.observe(this) { event: TruvideoSdkCameraEvent ->
            when (event.type) {
                TruvideoSdkCameraEventType.RecordingStarted -> {
                    val data = event.data as TruvideoSdkCameraEventRecordingStarted
                }
                TruvideoSdkCameraEventType.RecordingFinished -> {
                    val data = event.data as TruvideoSdkCameraEventRecordingFinished
                    data.media.filePath
                }
                TruvideoSdkCameraEventType.RecordingPaused -> TODO()
                TruvideoSdkCameraEventType.RecordingResumed -> TODO()
                TruvideoSdkCameraEventType.PictureTaken -> TODO()
                TruvideoSdkCameraEventType.CameraFlipped -> TODO()
                TruvideoSdkCameraEventType.ResolutionChanged -> TODO()
                TruvideoSdkCameraEventType.FlashModeChanged -> TODO()
                TruvideoSdkCameraEventType.ZoomChanged -> TODO()
                TruvideoSdkCameraEventType.MediaDeleted -> TODO()
                TruvideoSdkCameraEventType.MediaDiscard -> TODO()
