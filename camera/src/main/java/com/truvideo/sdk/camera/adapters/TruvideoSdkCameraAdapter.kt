...
class TruvideoSdkCameraAdapterImpl (
            ...
            object : CameraCaptureSession.CaptureCallback() {
                override fun onCaptureCompleted(
                    ...
                    if (afState == CaptureRequest.CONTROL_AF_STATE_INACTIVE) {
                        Log.d(TruvideoSdkCameraService.Companion.TAG, "Tap to focus: Fail")
                        focusLocked = true
                    } else if (validStated.contains(afState)) {
                        Log.d(TruvideoSdkCameraService.Companion.TAG, "Tap to focus: OK")
                        focusLocked = true
                    }
                    ...
                    if (focusLocked) {
                        ...
                        captureSession?.setRepeatingRequest(newRequest.build(), object : CameraCaptureSession.CaptureCallback() {
                            override fun onCaptureCompleted(
                                session: CameraCaptureSession,
                                request: CaptureRequest,
                                result: TotalCaptureResult
    ...
    override fun setZoomLevel(captureConfig: CameraCaptureConfig): Flow<Boolean> = callbackFlow {
        ...
        val surfaceList = requestSurfaces

        val captureRequest =
            captureConfig.toCaptureRequest(device, surfaceList).build()
    ...
    override fun startRecording(
        ...
        recordingSurface = with(recordingSetUp) {
            mediaRecorder.prepare(
                outputFile = File(videoPath),
    ...
    private val requestSurfaces: List<Surface>
        get() = if (isRecording) listOf(previewSurface!!, recordingSurface!!) else listOf(previewSurface!!)
